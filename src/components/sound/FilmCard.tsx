...
export default function FilmCard({
  ...
  const roles = sound.roles;
  const formats = sound.formats;
  const t = sound.filmCard;
  ...
  const titleViewportRef =
    useRef<HTMLDivElement>(null);

  const titleTextRef =
    useRef<HTMLHeadingElement>(null);

  const [titleScroll, setTitleScroll] =
    useState(0);
    ...
    >
      ...
      {film.link ? (
        <a
          ...
          aria-label={`${t.viewOnImdb}: ${film.title}`}
          onMouseEnter={() =>
          ...
          onMouseMove={(event) => {
            ...
            imdbTooltipRef.current.style.left =
              `${event.clientX + 14}px`;

            imdbTooltipRef.current.style.top =
              `${event.clientY + 14}px`;
          }}
        >
      ...
      <button
        ...
        className="w-full pt-3 text-left"
      >
